...
log = core.getLogger()
FLOOD_DELAY = 5
#default location /home/ubuntu/sr_lab3/IP_CONFIG
...
class OFHandler (EventMixin):
  def __init__ (self, connection, transparent):
    ...
    self.listenTo(core.sr_srhandler)
    core.sr_ofhandler.raiseEvent(RouterInfo(self.sw_info, self.rtable))
  ...
  def _handle_PacketIn (self, event):
    ...
    pkt = event.parse()
    raw_packet = pkt.raw
    core.sr_ofhandler.raiseEvent(SRPacketIn(raw_packet, event.port))
...
def launch (transparent=False):
  ...
  """    
  core.registerNew(sr_ofhandler, str_to_bool(transparent))
